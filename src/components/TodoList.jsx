...
import { deleteTodos,completeTodo } from '../reduxStore/todoSlice';
...
const TodoList = ({todoValue ,setIsEditing,setNewTodo,setNewCategory}) => {
    ...
    const [completed,setCompleted] =useState(todoValue.complete);
    ...
    const handleCompleted=()=>{
     // console.log("set completed");
      ...
      dispatch(completeTodo(todoValue.id));
    }
  return (

    <motion.li initial={{y:10,capacity:0}} animate={{y:0,opacity:1}} transition={{y:{type:'spring',stiffness:120},}} 
     onClick={()=>setMarked(!marked)} className={`w-full font-titleFont ng: text-xl border border-l-[6px] rounded-md py-2 cursor-pointer
      border-green-500 flex items-center justify-between pl-3 mb-4 ${completed ? 'bg-gray-500' : ''}`}  >
